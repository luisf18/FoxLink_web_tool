...
class IrKey extends devices_card {
    constructor(Addr) {
        super(Addr);
        ...
        this.parameters = {
            addr: { addr: FX_BASE.reg.ADDR, wg: new widget_int(this.id,"Addr",0,0,31), saved_value: this.Addr },
            name: { addr: IRKEY_REG.reg.NAME, wg: new widget_string(this.id,"name","IrKey",32), saved_value: 0 },
            CTRL: { addr: IRKEY_REG.reg.CTRL, wg: new widget_select(this.id,"log",1,{"Enable":2,"Disable":0}), saved_value: 0 },
            PROTOCOL_ST: { addr: IRKEY_REG.reg.PROTOCOL_ST, wg: new widget_select(this.id,"Start Protocols",1,{"SONY":1,"SONY and SAMSUNG":3}), saved_value: 1 },
    ...
    async get_firmware_version(){
        ...
        let fw1 = await fx.command( this.Addr, IRKEY_REG.cmd.FIRMWARE_ID  );
        let fw2 = await fx.command( this.Addr, IRKEY_REG.cmd.FIRMWARE_VERSION  );
        if( fw1.ok && fw1.ok ){
    ...
    async html(){
        ...
        const HTML = `
            <div class="user-header">
                <img src=${"devices/IrKey.png"}>
                <div>
                    <div class="device-title">${"IR_KEY"} [${this.id}]</div>
                    <div class="device-info">Modelo: ${"IR_KEY"} | Lote: ${this.lot} | V${parseInt(this.firmwareVersion/1000)}.${this.firmwareVersion%1000} </div>
                </div>
            </div>
    ...
    async default(){
        ...
        await fx.command_key(this.Addr,IRKEY_REG.cmd_write.RESTORE_KEPP_ADDR, true);
        await delay(20);
        await this.read();
        //for( const param in this.parameters ){
    ...
    async save(){
        ...
        let ans = await fx.command_key(this.Addr,IRKEY_REG.cmd_write.SAVE,true); // Comando de save
